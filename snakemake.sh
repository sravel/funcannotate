...
snakemake --cluster-config cluster_config.yml  --configfile config.yaml --jobs 200  --use-envmodules  --cluster "sbatch  -p {cluster.partition} --ntasks {cluster.ntasks} -c {cluster.cpus-per-task} -e {cluster.error} -o {cluster.output}"
